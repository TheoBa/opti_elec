...
def prepare_weather_df(weather_df):
    return (
        weather_df
        .rename(columns={"temperature_2m": "temperature_ext2"})
        .assign(date=lambda df: pd.to_datetime(df['date']))
        .set_index('date').resample('5min').ffill().reset_index(drop=False)
        .assign(
            day=lambda df: df['date'].dt.date,
            all_day_temperature=lambda df: df.groupby('day')['temperature_ext2'].transform('mean'),
            roll5_avg_temperature=lambda df: df['temperature_ext2'].rolling(window=5*20).mean(),
